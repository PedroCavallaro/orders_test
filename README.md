...
1. **Atomicidade:** Onde no código está garantida a atomicidade entre atualizar `orders` e inserir o evento na outbox?
  * A atomicidade está garantida dentro da transaction que atualiza a order a cria o outbox_event, isso está no arquivo order.repositoy.ts
...
3. **Idempotência:** Como você implementou a idempotência no consumer? Qual é a chave idempotente usada?
  * Salvo a chave do evento em processed_events como PK, e toda vez que um evento chega eu primeiro insiro ele na processed_events pra depois começar o tratamento. Dessa forma se der erro de PK, eu sei que aquele evento já passou pelo consumer, assim, não gero uma nova invoice, e trato esse evento duplicado 
